...
<div class="row">
  ...
  <div class="col-12 col-xs-12 col-sm-12 col-md-12 col-lg-12 container-header">
    <p class="text-muted text-left"><big>Reporte de pagos</big><img href="" src="{{asset('assets/img/bank.svg')}}" class="icon-header"></p>
  </div>
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    tablaArchivos += '<table class="display table table-bordered table-hover table-striped" id="tablaComplementos">'+
      ...
      '<tbody>';
      @foreach ($datos as $p)
      tablaArchivos+='<tr><td>{{ $p->clearing_document}}</td>'+
      '<td>{{ $p->fechabus}}</td>'+
      '<td>{{ $p->nombre_c}}</td>'+
      '<td>{{ $p->id_cliente}}</td>'+
      '<td>{{ $p->montoP}}</td>'+
     ...
     if($("#default-input-fecha1").val() == "" && $("#default-input-fecha2").val() == ""){
        ...
        tablaArchivos += '<table class="display table table-bordered table-hover table-striped" id="tablaComplementos">'+
          ...
          for (var i = 0; i < data.length; i++) {
            tablaArchivos += '<tr class="text-muted text-center" id="">'+
                    '<td id="archivos-recientes-Pago">'+data[i].clearing_document+'</td>'+
                    '<td>'+data[i].fechabus+'</td>'+
                    '<td>'+data[i].nombre_c+'</td>'+
                    '<td>'+data[i].id_cliente+'</td>'+
                    '<td>'+data[i].montoP+'</td>'+
                    '<td>'+data[i].monedaP+'</td>';
                  '</tr>' ;        
                  } 
          tablaArchivos += '</tbody>'+
        '</table>';
      ...
      $.ajax({
        ...
        tablaArchivos += '<table class="display table table-bordered table-hover table-striped" id="tablaComplementos">'+
          ...
          for (var i = 0; i < data.length; i++) {
            tablaArchivos += '<tr class="text-muted text-center" id="">'+
                    '<td id="archivos-recientes-tesoreria">'+data[i].clearing_document+'</td>'+
                    '<td>'+data[i].fechabus+'</td>'+
                    '<td>'+data[i].nombre_c+'</td>'+
                    '<td>'+data[i].id_cliente+'</td>'+
                    '<td>'+data[i].montoP+'</td>'+
